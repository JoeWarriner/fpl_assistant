...
def get_player_data():
    ...
    with open('etl/analysis_data.sql') as file:
        output = query(file.read())
        print(output)


    output.to_pickle('etl/temp_data/player_data.pkl')
...
def get_optimised_team():
    ...
    player_data = pd.read_pickle('etl/temp_data/player_data.pkl')

    player_data = player_data[player_data['full_name'] != 'Xhaka']
    player_data = player_data[player_data['full_name'] != 'Toney']
    player_data = player_data[player_data['full_name'] != 'Raya Martin']
    player_data = player_data[player_data['full_name'] != 'Kane']
    print(player_data)
    ...
    is_gk = player_data['element_type'] == 'GKP'
    ...
    model += pulp.lpSum([cost[i] * x[player] for i, player in enumerate(players)]) <= 830
